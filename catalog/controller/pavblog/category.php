 ...
 */
	class ControllerPavblogCategory extends Controller {
		...
		public function index() {  
			...
			if( !isset($this->request->get['pavblog/category']) ){
				$this->request->get['pavblog/category'] = null;
			}
			
			
			$parts = explode('_', (string)$this->request->get['pavblog/category']);
			...
			foreach( $children as $key => $sub ){
				...
				$sub['link']  =  $this->url->link( 'pavblog/category', 'pavblog/category=' .  $sub['category_id'] );
		...
		//	echo '<pre>'.print_r( $children,1 ); die;
			...
			if ($category_info) {
				...
				$this->data['breadcrumbs'][] = array(
					...
					'href'      => $this->url->link('pavblog/category', 'pavblog/category=' .  $category_id),      		
				...
				foreach( $blogs as $key => $blog ){
					...
					$blogs[$key]['category_link'] =  $this->url->link( 'pavblog/category', "pavblog/category=".$blog['category_id'] );
					...
					$blogs[$key]['link'] =  $this->url->link( 'pavblog/blog','pavblog/blog='.$blog['blog_id'] );
				}
				...
				$pagination->url = $this->url->link('pavblog/category', 'pavblog/category=' . $this->request->get['pavblog/category'] . $url . '&page={page}');
		...
		public function rss(){
			...
			$this->preload();
			if( isset($this->request->get['pavblog/category']) ){
				$id = (int)$this->request->get['pavblog/category'];
			} else {
				$id = 0;
			}
			...
			foreach( $blogs as $blog ){
				$link =  str_replace("&amp;","&",$this->url->link( 'pavblog/blog','pavblog/blog='.$blog['blog_id'] ));
				if( $blog['image'] ){
