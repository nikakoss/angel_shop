...
class ModelMenuMegamenu extends Model {		
		...
		if( $this->hasChild($parent) ){
			...
			foreach( $data as $menu ){
				...
				if( $this->hasChild($menu['megamenu_id']) || $menu['type_submenu'] == 'html'){
					...
					if($menu['show_title']) {
						$output .= '<span class="menu-title 13">'.$menu['title']."</span>";
					}
					if( $menu['description'] ){
					...
					if($menu['show_title']) {
						$output .= '<span class="menu-title 14">'.$menu['title']."</span>";
					}
					...
					if($menu['show_title']) {
						$output .= '<span class="menu-title 15">'.$menu['title']."</span>";
					}
	...
	public function genMegaMenuByConfig( $parentId, $level,$menu  ){
		...
		$output = '<li class="'.$menu['menu_class'].' parent '.$class.' " '.$this->renderAttrs($menu).'>
					...
					$output .= '<span class="menu-title 16">'.$menu['title']."</span>";
					if( $menu['description'] ){
						$output .= '<span class="menu-desc">' . $menu['description'] . "</span>";
					}
					if( $menu['image']){ $output .= '</span>';	}
		...
		if( $this->hasChild($menu['megamenu_id']) ){
			...
			if($product_total>0){
				...
				if( $menu['show_title'] ){
					...
					$t = '%s';
					if( $menu['image']){ $output .= '<span class="menu-icon" style="background:url(\''.$this->shopUrl."image/".$menu['image'].'\') no-repeat;">';	}
					$output .= '<span class="menu-title 11">'.$menu['title']."</span>";
					if( $menu['description'] ){
						$output .= '<span class="menu-desc">' . $menu['description'] . "</span>";
					}
					$output .= "<b class=\"caret\"></b>";
			...
			if($product_total>0){
				...
				if( $menu['show_title'] ){ 
					...
					if( $menu['image']){ $output .= '<span class="menu-icon" style="background:url(\''.$this->shopUrl."image/".$menu['image'].'\') no-repeat;">';	}
					$output .= '<span class="menu-title 12">'.$menu['title']."</span>";
					if( $menu['description'] ){
						$output .= '<span class="menu-desc">' . $menu['description'] . "</span>";
					}
					if( $menu['image']){ 
