  ...
  <div class="box">
    ...
    <div class="content">
				...
				?>	
					...
					?>
						<td class="left" style="90">
							<?php echo $tab_discount; ?>: <font style="font-weight:normal;font-size:80%;">(<?php echo str_replace(":","",$entry_customer_group." | ".$entry_quantity." | ".$entry_priority." | ".$entry_price);?>)</font>
						</td>
      ...
      <a onclick="add_related_option(false);" class="button"><?php echo $entry_add_related_options; ?></a>
			...
			<br><br>
			<span class="help">
				<?php echo $entry_ro_version.": ".$ro_version; ?> | <?php echo $text_ro_support; ?>
			</span>
	...
	if (isset($all_options)) {
		foreach ($all_options as $option_id => $option_data) {
			...
			foreach ($option_data['values'] as $option_value_id => $option_value_name) {
				echo "all_options[".$option_id."]['_".$option_value_id."'] = \"".$option_value_name."\";\n";
...
function add_related_option(params) {
  ...
  var str_add = "";
	...
	if (edit_columns == 100) {
		for (var i = 0; i < vopts_order.length; i++) {
			...
			str_add += "<div style=\"white-space: nowrap; float: left; margin-right:20px;\">";
			str_add += "<b>"+vopts[option_id]+"</b>:";
			str_add += "<select id='ro_o_"+ro_counter+"_"+option_id+"' name='relatedoptions["+ro_counter+"][options]["+option_id+"]' onChange='refresh_ro_status()'>";
			...
			//	if (option_value_id in all_options[option_id]) {
				...
				for (var str_option_value_id in all_options[option_id]) {
					var option_value_id = str_option_value_id.substr(1);
					//console.debug(option_value_id);
					str_add += "<option value='"+option_value_id+"'";
					if (params[option_id] == option_value_id) str_add +=" selected ";
					str_add += ">"+all_options[option_id][str_option_value_id]+"</option>";
				}
			//}
			str_add += "</select></div>";
		}
		...
		for (var i = 0; i < vopts_order.length; i++) {
			...
			if (edit_columns == 0 || (i+1)==vopts_order.length) {
				str_add += "</tr>";
			} else {
				if ( ((i+1)% edit_columns) == 0 ) {
					str_add += "</tr><tr>";
				}
			}
  ...
  if (params==false) refresh_ro_status();
...
function add_related_option_discount(ro_counter, discount) {
	...
	str_add += "<select name='"+first_name+"[customer_group_id]' title=\"<?php echo $entry_customer_group; ?>\">";
	<?php foreach ($customer_groups as $customer_group) { ?>
	...
	str_add += "<input type='text' size='2' name='"+first_name+"[quantity]' value='"+((discount=="")?(""):(discount['quantity']))+"' title=\"<?php echo $entry_quantity; ?>\">";
	str_add += "</td><td style='border:0px;'>";
	
	str_add += "<input type='text' size='2' name='"+first_name+"[priority]' value='"+((discount=="")?(""):(discount['priority']))+"' title=\"<?php echo $entry_priority; ?>\">";
	str_add += "</td><td style='border:0px;'>";
	
	str_add += "<input type='text' size='10' name='"+first_name+"[price]' value='"+((discount=="")?(""):(discount['price']))+"' title=\"<?php echo htmlspecialchars($entry_price); ?>\">";
	str_add += "</td><td style='border:0px;'>";
